...
public class UserController {
    ...
    public ResponseEntity<String> registerUser(@RequestBody SignupDto signupDto){
        System.out.println("RegisterUser API called with UserDto: " + signupDto);
        try{
            // 사용자 ID 중복 검사
            if(loginService.isUserIdExists(signupDto.getUserId())){
                return ResponseEntity.status(HttpStatus.CONFLICT).body("이미 존재하는 아이디입니다.");
            }

            // 사용자 ID 유효성 검사 -> 한글 입력 여부
            if (!loginService.isUserIdValid(signupDto.getUserId())){
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("사용할 수 없는 아이디입니다.");
            }

            // 이메일 유효성 검사 -> 올바른 이메일 형식인지 확인
            if(!loginService.isEmailValid(signupDto.getEmail())){
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("유효한 이메일 주소가 아닙니다.");
            }

            // 비밀번호 유효성 검사 -> 비밀번호 길이 제한
            if(!loginService.isPasswordValid(signupDto.getPassword())){
            ...
            User registeredUser = loginService.registerUser(signupDto);
